<?php
  ...
  if (isset($_POST['topluluk_id'])) {
    try {
       ...
       $konular = $db->prepare("select konular.*, (select count(*) from yorumlar where yorum_konu = konular.konu_id) as konu_yorum_sayisi, kullanicilar.kullanici_ad as konu_yazar_ad from konular inner join kullanicilar on konular.konu_yazar = kullanicilar.kullanici_id where konular.konu_topluluk = :topluluk_id order by konu_sabit desc, konu_id desc");
